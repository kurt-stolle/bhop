...
function PLAYER:HandleLeaderboards()
	if  self.notAllowedLeaderboards then return end

	local time = CurTime() - (self.StartTime or 0);
	local fails = self:Deaths();

	ES.DBQuery("INSERT INTO es_bhop_leaderboards SET steamid = '"..self:SteamID().."', time = "..tonumber(time)..", fails = "..fails..", map = '"..game.GetMap().."', difficulty = "..self:GetDifficulty()..", name = '"..ES.MySQLEscape(self:Nick()).."';")
...
function PLAYER:SendBest(best)
	...
	ES.DBQuery("SELECT MIN(time) AS best FROM es_bhop_leaderboards WHERE steamid = '"..self:SteamID().."' AND map = '"..game.GetMap().."' LIMIT 1;",function(dt)
...
local maps = {"Something","Bad","Happened","...","Tell","Excl"};
...
concommand.Add("bhop_requeststats",function(p,c,a)
	...
	local map = ES.MySQLEscape(a[1] or game.GetMap());
	...
	ES.DBQuery("SELECT time,fails,difficulty FROM es_bhop_leaderboards WHERE steamid = '"..p:SteamID().."' AND map = '"..map.."' ORDER BY time ASC LIMIT 15;",function(res)
...
concommand.Add("bhop_requestboards",function(p,c,a)
	...
	//ES.DBQuery("SELECT steamid,name,time FROM es_bhop_leaderboards WHERE map = '"..map.."' AND difficulty = 2 ORDER BY time ASC LIMIT 10;",function(res)
	ES.DBQuery("SELECT steamid, name, MIN(time) AS time FROM es_bhop_leaderboards WHERE difficulty = 2 AND map = '"..map.."' GROUP BY steamid ORDER BY time ASC LIMIT 10",function()
		if res and res[1] then
			boards[map][2] = res;
		end

		//ES.DBQuery("SELECT steamid,name,time FROM es_bhop_leaderboards WHERE map = '"..map.."' AND difficulty = 3 ORDER BY time ASC LIMIT 10;",function(res)
		ES.DBQuery("SELECT steamid, name, MIN(time) AS time FROM es_bhop_leaderboards WHERE difficulty = 3 AND map = '"..map.."' GROUP BY steamid ORDER BY time ASC LIMIT 10",function()
			if res and res[1] then
				boards[map][3] = res;
			end

			//ES.DBQuery("SELECT steamid,name,time FROM es_bhop_leaderboards WHERE map = '"..map.."' AND difficulty = 4 ORDER BY time ASC LIMIT 10;",function(res)
			ES.DBQuery("SELECT steamid, name, MIN(time) AS time FROM es_bhop_leaderboards WHERE difficulty = 4 AND map = '"..map.."' GROUP BY steamid ORDER BY time ASC LIMIT 10",function()
				if res and res[1] then
					boards[map][4] = res;
				end
